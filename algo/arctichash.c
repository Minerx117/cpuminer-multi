...
void arctichash_hash(void *output, const void *input)
{
    ...
    LYRA2RE(hash + 3*nOutLen, nOutLen, hash + 2*nOutLen, nOutLen, hash + 2*nOutLen, nOutLen, 1, 8, 8);
    ...
    LYRA2RE(hash + 9*nOutLen, nOutLen, hash + 8*nOutLen, nOutLen, hash + 8*nOutLen, nOutLen, 1, 8, 8);
    ...
    LYRA2RE(output, 32, hash + 25*nOutLen, nOutLen, hash + 25*nOutLen, nOutLen, 1, 8, 8);
...
int scanhash_arctichash(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	do {
		...
		arctichash_hash(hash32, endiandata);
